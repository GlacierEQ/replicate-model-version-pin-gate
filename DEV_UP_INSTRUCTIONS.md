...
**Repository:** `GlacierEQ/replicate-model-version-pin-gate`  
**Company lens (independent):** Replicate (`replicate`)  
**Innovation:** Model Version Pin Gate  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Replicate affiliation, proprietary access, or production deployment.
...
Require explicit model version pins; reject floating tags for production-class calls.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Replicate employment, endorsement, or proprietary systems access.
...
File: `src/model_version_pin_gate.py`

- Expand `ModelVersionPinGate` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_model_version_pin_gate.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_model_version_pin_gate.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Replicate APIs
